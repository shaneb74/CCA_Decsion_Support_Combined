...
def get_gc_summary():
    """
    Stub: pull GC outputs from your existing store.
    Return None if GC wasn't completed.
    Expected keys: care_path (str), flags (list[str]), zip (str|None)
    """
...
def get_calculator_snapshot():
    """
    Stub: pull Calculator outputs from your existing store.
    Return None if Calculator wasn't completed.
    Expected keys:
      income (dict), benefits (dict), assets (dict),
      monthly_cost (float), monthly_gap (float), years_funded (float|None),
      payer_hint (str|None)
    """
...
def read_only_badges(flags: list[str]):
    ...
    for i, f in enumerate(flags):
        with cols[i % len(cols)]:
            st.markdown(f"<div style='display:inline-block;padding:6px 10px;border-radius:999px;border:1px solid #ddd;font-size:0.85rem;'>🛈 {f.replace('_',' ').title()}</div>", unsafe_allow_html=True)


# --------------------------
# App chrome
# --------------------------
st.set_page_config(page_title="Plan for My Advisor (PFMA) – Mock", page_icon="📞", layout="centered")
...
if step == 1:
    with st.expander("Your current plan summary", expanded=True):
        ...
        if calc:
            ...
            col1.metric("Est. Monthly Cost", f"${calc.get('monthly_cost','—')}")
            col2.metric("Monthly Gap", f"${calc.get('monthly_gap','—')}")
            yf = calc.get('years_funded')
            col3.metric("Years Funded", f"{yf:.1f}" if isinstance(yf,(int,float)) else "—")
        else:
    ...
    with st.form("pfma_booking"):
        ...
        relationship = st.selectbox("Your relationship to the care recipient", ["Self", "Spouse/Partner", "Adult Child", "POA/Representative", "Other"], index=["Self","Spouse/Partner","Adult Child","POA/Representative","Other"].index(rel_default) if rel_default in ["Self","Spouse/Partner","Adult Child","POA/Representative","Other"] else 0)
...
if step == 2:
    ...
    st.link_button("Back to home", "#", help="In your app this would return to the main hub.")

# asset_engine.py — Household & Budget drawers with clear canonical benefit flags
...
class IncomeAssetsEngine:
    ...
    def _section_benefits(self, names: list[str]):
        with st.expander("Benefits (VA, Long-Term Care insurance)", expanded=True):
            ...
            # canonical flags PFMA can trust
            st.session_state["benefits_va"] = bool((a_va + b_va) > 0)
            st.session_state["benefits_ltc"] = (a_ltc == "Yes") or (b_ltc == "Yes")

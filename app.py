...
# CSS for confirm buttons and gamification animations + tools cards
st.markdown("""
    <style>
    ...
    .stAlert { animation: fadeIn 0.5s; }
    .badge { animation: fadeIn 0.5s; }
...
def _render_pfma_tools_block():
    ...
    s = st.session_state
    # Centralized totals
    income = totals.get_income_totals(s)
    costs = totals.get_cost_totals(s)
    assets = totals.get_asset_totals(s)
    picture = totals.compute_picture(s)
    ...
    with colA:
        ...
        for k in ("inc_A","inc_B","inc_house","va_A","va_B","rm_monthly","total"):
            writer.writerow(["Income", k, income.get(k, 0)])
        for k in ("care_total","home_monthly","mods_monthly","other_monthly","total"):
            writer.writerow(["Costs", k, costs.get(k, 0)])
        writer.writerow(["Assets","common", assets["common"]])
        writer.writerow(["Assets","detail", assets["detail"]])
        writer.writerow(["Assets","home_sale_proceeds_applied", assets["sale_proceeds"]])
        writer.writerow(["Assets","reverse_mortgage_lump_applied", assets["rm_lump"]])
        writer.writerow(["Assets","rm_fees_out_of_pocket", assets["rm_fees_oop"]])
        writer.writerow(["Assets","total_effective", assets["total_effective"]])
        writer.writerow(["Picture","gap", picture["gap"]])
        writer.writerow(["Picture","runway_months", picture["months_runway"]])
        ...
        html_parts = [
            ...
            _row("Individual A", income["inc_A"]),
            _row("Individual B", income["inc_B"]),
            _row("Household", income["inc_house"]),
            _row("VA — A", income["va_A"]),
            _row("VA — B", income["va_B"]),
            _row("Reverse mortgage (monthly)", income["rm_monthly"]),
            f"<tr><th>Total</th><th>{_m(income['total'])}</th></tr>", "</table>",
            '<h3>Monthly Costs</h3><table><tr><th>Category</th><th>Monthly</th></tr>',
            _row("Care", costs["care_total"]),
            _row("Home", costs["home_monthly"]),
            _row("Home modifications", costs["mods_monthly"]),
            _row("Other", costs["other_monthly"]),
            f"<tr><th>Total</th><th>{_m(costs['total'])}</th></tr>", "</table>",
            '<h3>Assets</h3><table><tr><th>Assets</th><th>Amount</th></tr>',
            _row("Common", assets["common"]),
            _row("Less common", assets["detail"]),
            _row("Home sale net proceeds (applied)", assets["sale_proceeds"]),
            _row("Reverse mortgage lump (applied)", assets["rm_lump"]),
            f"<tr><td>RM fees out-of-pocket (deducted)</td><td>- {_m(assets['rm_fees_oop'])}</td></tr>",
            f"<tr><th>Assets Total (effective)</th><th>{_m(assets['total_effective'])}</th></tr>", "</table>"
        ]
        if picture["gap"] > 0:
            years = picture["years"]; rem = picture["rem"]
            ym = f"{years} years {rem} months" if years else f"{rem} months"
            html_parts.append(f"<p><b>Monthly gap:</b> {_m(picture['gap'])}. <b>Estimated runway:</b> {ym}.</p>")
        else:
            html_parts.append(f"<p><b>Monthly surplus:</b> {_m(-picture['gap'])}.</p>")
    ...
    with colB:
        ...
        preview = {
            ...
            "context": {
                ...
                "gap": picture["gap"],
                "assets_total_effective": assets["total_effective"],
        ...
        if st.button("Send to AI Agent (mock)", key="pfma_send_v2"):
            opener = f"I see total monthly income {_m(income['total'])} and costs {_m(costs['total'])}. "
            if picture["gap"] <= 0:
                opener += f"You have a monthly surplus of {_m(-picture['gap'])}."
            else:
                years = picture["years"]; rem = picture["rem"]
                if years or rem:
                    opener += f"You have a monthly gap of {_m(picture['gap'])}. Effective assets {_m(assets['total_effective'])} cover about {years}y {rem}m at current spend."
                else:
                    opener += f"You have a monthly gap of {_m(picture['gap'])}."
...
def render_pfma():
    ...
    # ---- Tools & AI Agent (PLACED HERE, below booking) ----
    ...
    if ENABLE_PFMA_GAMIFICATION:
        ...
        progress = filled_count / max(total_sections, 1)
    ...
    if ENABLE_PFMA_GAMIFICATION:
        ...
        if 'filled_count' in locals() and filled_count == total_sections:
...
"""
    ...
    cost_totals = totals.get_cost_totals(s)
    home_monthly = cost_totals["home_monthly"]
    mods_monthly = cost_totals["mods_monthly"]
    other_monthly = cost_totals["other_monthly"]
    ...
    # Monthly Income (centralized; includes RM monthly)
    st.subheader("Monthly Income")
    income = totals.get_income_totals(s)
    st.table([
        {"Source":"Individual A (SS + Pension + Other)","Monthly":money(income["inc_A"])},
        {"Source":"Individual B (SS + Pension + Other)","Monthly":money(income["inc_B"])},
        {"Source":"Household / Shared (rent, annuity, investments, trust, other)","Monthly":money(income["inc_house"])},
        {"Source":"VA — A","Monthly":money(income["va_A"])},
        {"Source":"VA — B","Monthly":money(income["va_B"])},
        {"Source":"Reverse mortgage (monthly)","Monthly":money(income["rm_monthly"])},
        {"Source":"Total Income","Monthly":money(income["total"])},
    ])

    # Totals + runway
    st.subheader("Totals")
    picture = totals.compute_picture(s)
    monthly_need = cost_totals["total"]
    gap = picture["gap"]
    assets = totals.get_asset_totals(s)

    col1, col2, col3 = st.columns(3)
    col1.metric("Monthly Care + Selected Costs", money(monthly_need))
    col2.metric("Total Monthly Income (incl. VA & RM)", money(income["total"]))
    col3.metric("Estimated Monthly Gap", money(gap))
    if gap > 0 and assets["total_effective"] > 0:
        years = picture["years"]; rem = picture["rem"]

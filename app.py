...
"""
    ...
    st.caption("Subtotals update as you type")
    st.table([
    ...
    st.subheader("Assets (for runway)")
    st.table([
    ...
    if totals["gap"] <= 0:
        ...
        if totals["assets_total_effective"] > 0:
            runway_text = f"{totals['years']} years {totals['rem']} months" if totals["years"] > 0 else f"{totals['rem']} months"

...
# --- compute_totals (read-only, robust to key synonyms) ---
...
def compute_totals(state):
    s = state
    # Income
    inc_A = _to_int_safe(_g(s, "inc_A"), 0) or (_to_int_safe(_g(s,"a_ss")) + _to_int_safe(_g(s,"a_pn")) + _to_int_safe(_g(s,"a_other")))
    inc_B = _to_int_safe(_g(s, "inc_B"), 0) or (_to_int_safe(_g(s,"b_ss")) + _to_int_safe(_g(s,"b_pn")) + _to_int_safe(_g(s,"b_other")))
    inc_house = _to_int_safe(_g(s,"inc_house"),0) or (_to_int_safe(_g(s,"hh_rent")) + _to_int_safe(_g(s,"hh_annuity")) + _to_int_safe(_g(s,"hh_invest","hh_investments")) + _to_int_safe(_g(s,"hh_trust")) + _to_int_safe(_g(s,"hh_other")))
    va_A = _to_int_safe(_g(s,"va_A","a_va_monthly"))
    va_B = _to_int_safe(_g(s,"va_B","b_va_monthly"))
    rm_monthly = _to_int_safe(_g(s,"rm_monthly","rm_monthly_income"))
    income_total = inc_A + inc_B + inc_house + va_A + va_B + rm_monthly

    # Costs
    care_total = _to_int_safe(_g(s,"care_total","care_monthly_total"))
    home_monthly = _to_int_safe(_g(s,"home_monthly","home_monthly_total"))
    mods_monthly = _to_int_safe(_g(s,"mods_monthly","mods_monthly_total"))
    other_monthly = _to_int_safe(_g(s,"other_monthly","other_monthly_total"))
    monthly_costs_total = care_total + home_monthly + mods_monthly + other_monthly

    # Assets
    assets_common = _to_int_safe(_g(s,"assets_common","assets_common_total"))
    assets_detail = _to_int_safe(_g(s,"assets_detail","assets_detailed_total","assets_less_common_total"))
    ...
    sale_proceeds = _to_int_safe(_g(s,"sale_proceeds","home_sale_net_proceeds","sale_net")) if apply_sale else 0
    rm_lump = _to_int_safe(_g(s,"rm_lump","rm_lump_applied"))
    rm_fees_oop = _to_int_safe(_g(s,"rm_fees_oop","rm_fees_oop_total"))
    mods_upfront = _to_int_safe(_g(s,"mods_upfront","mods_upfront_total"))
...
def render_pfma():
    ...
    # Tools block (Exports + AI handoff)
    try:
...
"""
    ...
    home_monthly = int(t.get("home_monthly", 0))
    mods_monthly = int(t.get("mods_monthly", 0))
    other_monthly = int(t.get("other_monthly", 0))
    ...
    inc_A = int(t.get("inc_A",0))
    inc_B = int(t.get("inc_B",0))
    inc_house = int(t.get("inc_house",0))
    va_A = int(t.get("va_A",0)); va_B = int(t.get("va_B",0))
    income_total = int(t.get("income_total", inc_A + inc_B + inc_house + va_A + va_B))
    st.table([

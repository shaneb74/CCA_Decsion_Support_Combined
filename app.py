# app.py — charts disabled build (safe)
...
# ---- Chart switches ----
ENABLE_CHARTS = False  # flip to True later if you want to re-enable visuals
USE_MPL = False        # force-disable matplotlib completely
try:
...
def donut_altair(labels, values, title):
    import pandas as pd
    if not ENABLE_CHARTS or not HAVE_ALTAIR:
        return None
...
# ---------- Details (numeric fallback; charts disabled) ----------
    ...
    if ENABLE_CHARTS and HAVE_ALTAIR:
        chart1 = donut_altair(cost_labels, cost_values, 'Costs (monthly)')
        if chart1: st.altair_chart(chart1, use_container_width=False)
        chart2 = donut_altair(inc_labels, inc_values, 'Income (monthly)')
        if chart2: st.altair_chart(chart2, use_container_width=False)
        chart3 = donut_altair(['Household assets'], [assets_total], 'Assets (total)')
        if chart3: st.altair_chart(chart3, use_container_width=False)
    else:
        st.info('Charts are temporarily disabled. Showing numeric breakdown instead.')
        st.write('**Costs (monthly):**')
        st.write({l: int(v) for l, v in zip(cost_labels, cost_values)})
        st.write('**Income (monthly):**')
        st.write({l: int(v) for l, v in zip(inc_labels, inc_values)})
        st.write('**Assets (total):**')
        st.write({'Household assets': int(assets_total)})

    st.caption('Numbers update as you adjust values in Household & Budget.')

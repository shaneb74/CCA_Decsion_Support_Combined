...
# Household drawers optional (benefits/VA/LTC live here)
try:
    import asset_engine
except Exception:
    asset_engine = None
...
# ---------- PFMA (inline page, no page links so state survives) ----------
def _prefill_pfma_flags_and_conditions():
    """
    Populate:
      - pfma_va (bool) from a_va_monthly / b_va_monthly
      - pfma_ltc (bool) from has_ltc_insurance or related keys
      - pfma_conditions (list[str]) merged from AL/MC condition multiselects per person
    Only sets defaults if user hasn't already interacted with PFMA.
    """
    s = st.session_state

    # VA detection
    va_detected = any([
        int(s.get("a_va_monthly", 0) or 0) > 0,
        int(s.get("b_va_monthly", 0) or 0) > 0,
        int(s.get("va_monthly", 0) or 0) > 0,
    ])
    if "pfma_va" not in s:
        s.pfma_va = bool(va_detected)

    # LTC detection (prefer the canonical flag set by asset_engine)
    ltc_detected = bool(s.get("has_ltc_insurance", False))
    if not ltc_detected:
        # fallbacks: any session key with "ltc" truthy, or PFMA payer already chosen
        for k, v in s.items():
            if "ltc" in str(k).lower():
                if str(v).lower() not in ("false", "0", "", "none", "no"):
                    ltc_detected = True
                    break
    ...
    # Merge chronic conditions captured in Cost Planner panels
    merged: list[str] = []
    ...
    for p in s.get("people", []):
        pid = p.get("id")
        # New multiselects saved by cost_controls
        for k in (f"al_conditions_{pid}", f"mc_conditions_{pid}"):
            vals = s.get(k)
            if isinstance(vals, list) and vals:
                _add_unique(vals)
        # Legacy single selects (map to specific items where possible)
...
def render_pfma():
    _prefill_pfma_flags_and_conditions()
    ...
    with st.expander("Your current plan summary", expanded=True):
        ...
        if people and recs:
            ...
            for p in people:
                ...
                rec = recs.get(pid)
                chosen = overrides.get(pid, getattr(rec, "care_type", "in_home"))
                st.write(f"**{name}:** {nice.get(chosen, chosen).title()}")
        else:
    ...
    # --- Booking first (primary objective) ---
    ...
    with colA:
        if st.button("Back to Home", key="pfma_back_home_btn", type="secondary"):
            s.step = "intro"
            st.rerun()
    with colB:
    ...
    with colC:
        if st.button("Finish", key="pfma_finish_btn", type="secondary"):
            s.step = "intro"
            st.rerun()

    # Optional appears after booking
    ...
    st.divider()
    if st.button("Save optional details", key="pfma_optional_save_btn"):
        s.pfma_optional = {
...
if st.sidebar.button("Schedule with an Advisor", use_container_width=True, key="pfma_sidebar_btn"):
...
"""
    ...
    c1, c2 = st.columns(2)
    with c1:
        if st.button("Start", key="intro_start_btn"):
    ...
    if st.button("Continue", key="aud_continue_btn"):
    ...
    with c1:
        ...
        if st.button(f"No, just plan for **{primary}**", key="spouse_no_btn"):
            st.session_state.step = "planner"; st.rerun()
    with c2:
        if st.button("Add spouse/partner and continue", key="spouse_yes_btn", disabled=not st.session_state.get("care_partner_add", False)):
    ...
    if st.button("Save and continue", key="planner_save_btn"):
        if not answers:
    ...
    if st.button(f"Start {name}'s care plan", key="trans_start_btn"):
    ...
    valid_options = ["none", "in_home", "assisted_living", "memory_care"]
    ...
    for p in s.get("people", []):
        ...
        # Defensive fallback to avoid NameError and bad indices
        if care_type not in valid_options:
            care_type = "in_home"
        ...
        idx = valid_options.index(care_type)
        label = st.selectbox(
            f"Care scenario for {name}",
            [nice[o] for o in valid_options],
            index=idx,
            key=f"override_{pid}",
        )
    ...
    with c1:
        if st.button("Back to questions", key="rec_back_questions_btn"):
            st.session_state.step = "planner"; st.rerun()
    with c2:
        if st.button("See Costs", key="rec_see_costs_btn"):
            st.session_state.step = "calculator"; st.rerun()
    with c3:
        if st.button("Finish", key="rec_finish_btn"):
    ...
    c1, c2, c3 = st.columns(3)
    with c1:
        if st.button("Back to recommendations", key="calc_back_rec_btn"):
            st.session_state.step = "recommendations"; st.rerun()
    with c2:
        if st.button("Add Household & Assets", key="calc_household_btn"):
    ...
    c1, c2, c3 = st.columns(3)
    with c1:
        if st.button("Back to Costs", key="hh_back_costs_btn"):
            st.session_state.step = "calculator"; st.rerun()
    with c2:
        if st.button("View Detailed Breakdown", key="hh_breakdown_btn"):
            st.session_state.step = "breakdown"; st.rerun()
    with c3:
        if st.button("Finish", key="hh_finish_btn"):
            st.session_state.step = "intro"; st.rerun()
    ...
    with cta1:
        if st.button("Back to Household", key="bd_back_house_btn"): st.session_state.step = "household"; st.rerun()
    with cta2:

...
def compute_totals(s):
    ...
    rm_monthly = _to_int(s.get("rm_monthly_income"))
    ...
    rm_lump = _to_int(s.get("rm_lump_applied"))
    rm_fees_oop = _to_int(s.get("rm_fees_oop_total"))
    ...
    return {
        ...
        "assets_common": assets_common, "assets_detail": assets_detail, "sale_proceeds": sale_proceeds, "rm_lump": rm_lump, "rm_fees_oop": rm_fees_oop,
...
# --- Export payload (read-only) ---
def build_export_payload():
    s = st.session_state
    t = compute_totals(s)
    payload = {
        "income": {
            "individual_A": t["inc_A"],
            "individual_B": t["inc_B"],
            "household": t["inc_house"],
            "va_A": t["va_A"],
            "va_B": t["va_B"],
            "reverse_mortgage_monthly": t["rm_monthly"],
            "total": t["income_total"],
        },
        "costs": {
            "care": t["care_total"],
            "home": t["home_monthly"],
            "mods_monthly": t["mods_monthly"],
            "other": t["other_monthly"],
            "total": t["monthly_costs_total"],
        },
        "assets": {
            "common": t["assets_common"],
            "less_common": t["assets_detail"],
            "home_sale_proceeds_applied": t["sale_proceeds"],
            "reverse_mortgage_lump_applied": t["rm_lump"],
            "rm_fees_out_of_pocket": t["rm_fees_oop"],
            "mods_upfront_deducted": t["mods_upfront"] if t["mods_deduct"] else 0,
            "total_effective": t["assets_total_effective"],
        },
        "picture": {
            "gap": t["gap"],
            "runway_months": t["months_runway"],
            "runway_years": t["years"],
            "runway_months_remainder": t["rem"],
        },
        "meta": {
...
# --- Export UI (CSV + HTML "print view") ---
def render_export_ui():
    if not EXPORT_ENABLED: 
        return
    try:
        payload = build_export_payload()
        # CSV: flatten income, costs, assets
        csv_buf = StringIO()
        ...
        inc = payload["income"]
        html += [f"<tr><td>Individual A</td><td>{money(inc['individual_A'])}</td></tr>",
                 f"<tr><td>Individual B</td><td>{money(inc['individual_B'])}</td></tr>",
                 f"<tr><td>Household</td><td>{money(inc['household'])}</td></tr>",
                 f"<tr><td>VA — A</td><td>{money(inc['va_A'])}</td></tr>",
                 f"<tr><td>VA — B</td><td>{money(inc['va_B'])}</td></tr>",
                 f"<tr><td>Reverse mortgage (monthly)</td><td>{money(inc['reverse_mortgage_monthly'])}</td></tr>",
                 f"<tr><th>Total</th><th>{money(inc['total'])}</th></tr>",
                 "</table>"]
        html.append('<h3>Monthly Costs</h3><table><tr><th>Category</th><th>Monthly</th></tr>')
        c = payload["costs"]
        html += [f"<tr><td>Care</td><td>{money(c['care'])}</td></tr>",
                 f"<tr><td>Home</td><td>{money(c['home'])}</td></tr>",
                 f"<tr><td>Home modifications</td><td>{money(c['mods_monthly'])}</td></tr>",
                 f"<tr><td>Other</td><td>{money(c['other'])}</td></tr>",
                 f"<tr><th>Total</th><th>{money(c['total'])}</th></tr>",
                 "</table>"]
        html.append('<h3>Assets</h3><table><tr><th>Assets</th><th>Amount</th></tr>')
        a = payload["assets"]
        html += [f"<tr><td>Common</td><td>{money(a['common'])}</td></tr>",
                 f"<tr><td>Less common</td><td>{money(a['less_common'])}</td></tr>",
                 f"<tr><td>Home sale net proceeds (applied)</td><td>{money(a['home_sale_proceeds_applied'])}</td></tr>",
                 f"<tr><td>Reverse mortgage lump (applied)</td><td>{money(a['reverse_mortgage_lump_applied'])}</td></tr>",
                 f"<tr><td>RM fees out-of-pocket (deducted)</td><td>- {money(a['rm_fees_out_of_pocket'])}</td></tr>",
                 f"<tr><th>Assets Total (effective)</th><th>{money(a['total_effective'])}</th></tr>",
                 "</table>"]
        pic = payload["picture"]
        ...
        st.download_button("Export CSV", data=csv_bytes, file_name="senior_navigator_export.csv", mime="text/csv")
        st.download_button("Export Print View (HTML)", data=html_bytes, file_name="senior_navigator_export.html", mime="text/html")
    except Exception as e:
        st.warning(f"Export unavailable: {e}")

# --- Snapshots UI (A/B compare) ---
def render_snapshots_ui():
    if not SNAPSHOTS_ENABLED:
        return
    try:
        s = st.session_state
        snaps = s.get("snapshots_derived", [])
        st.caption("Save the current totals as a snapshot and compare scenarios.")
        colA, colB = st.columns([2,1])
        with colA:
            label = st.text_input("Label for snapshot", value=s.get("last_snapshot_label","Baseline"), key="snapshot_label_input")
        with colB:
            if st.button("Save snapshot"):
                payload = build_export_payload()
                snap = {"label": label, "payload": payload}
                snaps.append(snap)
                s["snapshots_derived"] = snaps
                s["last_snapshot_label"] = label
                st.success(f"Saved snapshot: {label}")
        if len(snaps) >= 2:
            st.subheader("Compare snapshots")
            names = [sn["label"] for sn in snaps]
            c1, c2 = st.columns(2)
            idx1 = c1.selectbox("Left", names, index=len(names)-2, key="snap_left_idx")
            idx2 = c2.selectbox("Right", names, index=len(names)-1, key="snap_right_idx")
            left = snaps[names.index(idx1)]["payload"]
            right = snaps[names.index(idx2)]["payload"]
            def money(n): 
                try: return f"${int(n):,}"
                except: return str(n)
            st.write("**Monthly Income**")
            for k in ["individual_A","individual_B","household","va_A","va_B","reverse_mortgage_monthly","total"]:
                lv, rv = left["income"][k], right["income"][k]
                st.write(f"- {k}: {money(lv)} → {money(rv)}")
            st.write("**Monthly Costs**")
            for k in ["care","home","mods_monthly","other","total"]:
                lv, rv = left["costs"][k], right["costs"][k]
                st.write(f"- {k}: {money(lv)} → {money(rv)}")
            st.write("**Assets**")
            for k in ["common","less_common","home_sale_proceeds_applied","reverse_mortgage_lump_applied","rm_fees_out_of_pocket","total_effective"]:
                lv, rv = left["assets"][k], right["assets"][k]
                st.write(f"- {k}: {money(lv)} → {money(rv)}")
            st.write("**Financial Picture**")
            lv_gap, rv_gap = left["picture"]["gap"], right["picture"]["gap"]
            st.write(f"- gap: {money(lv_gap)} → {money(rv_gap)}")
    except Exception as e:
        st.warning(f"Snapshots unavailable: {e}")

# --- Sensitivity (what-if) ---
def render_sensitivity_ui():
    ...
    try:
        ...
        s = st.session_state
        t = compute_totals(s)
        with st.expander("What-if (sensitivity)", expanded=False):
            col1, col2 = st.columns(2)
            with col1:
                infl = st.number_input("Inflation on costs (%/yr)", min_value=0.0, max_value=20.0, value=3.0, step=0.5, key="sens_infl")
                cola = st.number_input("COLA on Social Security (%/yr)", min_value=0.0, max_value=20.0, value=2.0, step=0.5, key="sens_cola")
            with col2:
                step_up = st.number_input("Care escalation (+$/mo)", min_value=0, value=0, step=100, key="sens_stepup")
                step_month = st.number_input("Escalation starts after (months)", min_value=0, value=12, step=1, key="sens_stepmonth")
            # Simple 12-month projection to show impact on runway
            months = 12
            costs_proj = int(t["monthly_costs_total"] * (1 + infl/100.0)) + int(step_up if step_month <= 12 and step_up>0 else 0)
            ss_base = _to_int(s.get("a_ss")) + _to_int(s.get("b_ss"))
            ss_proj = int(ss_base * (1 + cola/100.0)) + (t["income_total"] - ss_base)  # COLA only on SS lines
            gap_proj = costs_proj - ss_proj
            if gap_proj <= 0:
                st.info("What-if result: projected monthly surplus (no asset drawdown).")
            else:
                months_runway = (t["assets_total_effective"] // gap_proj) if t["assets_total_effective"] > 0 else 0
                yrs, rem = (months_runway // 12, months_runway % 12) if months_runway else (0, 0)
                st.info(f"What-if result: projected gap {gap_proj:,} → projected runway {yrs} years {rem} months")
    except Exception as e:
        st.warning(f"Sensitivity unavailable: {e}")

# --- AI Handoff (mock chat) ---
def render_ai_handoff_chat(context_label=""):
    ...
    try:
        ...
        with st.expander(f"AI Agent (mock) — {context_label}", expanded=True):
            st.markdown("**You:** Discuss this with my AI Agent")
            if t["gap"] <= 0:
                opening = f"I see total monthly income {money(t['income_total'])} and costs {money(t['monthly_costs_total'])}. You have a monthly surplus of {money(-t['gap'])}."
            else:
                opening = f"I see total monthly income {money(t['income_total'])} and costs {money(t['monthly_costs_total'])}. You have a monthly gap of {money(t['gap'])}."
            st.markdown(f"**Agent:** {opening} Would you like to explore options for your home decision ('{st.session_state.get('home_decision','')}') or care scenario next?")
            st.caption("This is a mock preview of how a live AI agent could pick up your results.")
    except Exception as e:
        st.warning(f"AI handoff unavailable: {e}")

# --- Breakdown enhancements (call at end of Breakdown page) ---
def render_breakdown_enhancements():
    render_export_ui()
    render_snapshots_ui()

...
# ---- PFMA Tools Block (bundled definitive) ----
def _render_pfma_tools_block():
    """
    Exports + AI Agent handoff for the PFMA page.
    - Read-only: only reads st.session_state, no writes except a temporary guard key.
    - Safe: unique widget keys; render-once guard is controlled by caller.
    """

    # Render-once guard: caller may reset this per run (we do that in render_pfma)
...
def render_pfma():
    ...
    # Keep tools visible every run (prototype)
    st.session_state["_pfma_tools_rendered"] = False
    with st.expander("Tools & AI Agent", expanded=False):

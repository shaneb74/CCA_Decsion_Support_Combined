...
# === Consolidated Tools (v2): Export + AI handoff with realistic dialogue ===
def _money_fmt(n):
    try: 
        return f"${int(n):,}"
    except Exception:
...
def _render_tools_section_v2():
    try:
        if not (EXPORT_ENABLED or AI_HANDOFF_ENABLED):
            return
        payload = _build_export_payload_v2()
        ...
        with colA:
            ...
            if EXPORT_ENABLED:
                ...
                st.download_button("Export CSV", data=csv_buf.getvalue().encode("utf-8"), file_name="senior_navigator_export.csv", mime="text/csv", key="dl_csv_tools_v2")
                ...
                st.download_button("Export Print View (HTML)", data=html_bytes, file_name="senior_navigator_export.html", mime="text/html", key="dl_html_tools_v2")
                st.caption("Print-friendly; use your browser to save as PDF.")

        with colB:
            st.markdown("#### AI Agent handoff")
            if AI_HANDOFF_ENABLED:
                preset = st.selectbox("Prompt preset", ["Close the funding gap","Compare housing options","Maximize benefits","Explain my numbers","Tune in-home care hours"], key="ai_preset_v2")
                # Show copyable prompt preview
                prompt_preview = {
                    "goal": preset,
                    "context": {
                ...
                st.text_area("Prompt that would be sent", value=json.dumps(prompt_preview, indent=2), height=180, key="ai_prompt_box_v2")
                # Adaptive opener + chips
                if st.button("Send to AI Agent (mock)", key="ai_send_btn_v2"):
                    # Initialize transcript once
                    transcript = st.session_state.get("ai_transcript_local", [])
                    transcript.append(("user", preset))
                    # Build adaptive opener
                    opener = f"I see total monthly income {_money_fmt(inc['total'])} and costs {_money_fmt(cst['total'])}. "
                    if pic["gap"] <= 0:
                        opener += f"You have a monthly surplus of {_money_fmt(-pic['gap'])}. "
                        opener += "We can explore quality upgrades, reserves, or pacing asset use."
                    else:
                        opener += f"You have a monthly gap of {_money_fmt(pic['gap'])}. "
                        opener += f"Effective assets are {_money_fmt(ast['total_effective'])}, roughly covering {pic['runway_years']}y {pic['runway_months_remainder']}m at current burn."
                    if meta.get("home_decision"):
                        opener += f" You selected **{meta['home_decision']}**"
                        if meta.get("rm_plan"):
                            opener += f" with **{meta['rm_plan']}** plan"
                        opener += "."
                    transcript.append(("agent", opener))
                    st.session_state["ai_transcript_local"] = transcript
                # Action chips (buttons)
                chip_cols = st.columns(3)
                chips = [("Close the funding gap","gap"),
                         ("Compare housing options","housing"),
                         ("Maximize benefits (VA, others)","benefits"),
                         ("Explain my numbers","explain"),
                         ("Tune in-home care hours","hours")]
                for i, (label, code) in enumerate(chips[:3]):
                    if chip_cols[i].button(label, key=f"ai_chip_{code}"):
                        _ai_transcript_add_response(code, inc, cst, ast, pic, meta)
                chip_cols2 = st.columns(2)
                for i, (label, code) in enumerate(chips[3:]):
                    if chip_cols2[i].button(label, key=f"ai_chip2_{code}"):
                        _ai_transcript_add_response(code, inc, cst, ast, pic, meta)
        ...
        with st.expander("Conversation preview", expanded=False):
            transcript = st.session_state.get("ai_transcript_local", [])
            if not transcript:
                st.write("_No messages yet. Use the Send button or action chips above._")
            else:
...
def _ai_transcript_add_response(code, inc, cst, ast, pic, meta):
    transcript = st.session_state.get("ai_transcript_local", [])
    if code == "gap":
        msg = ("Top levers: (1) increase income (RM monthly, rental, annuity), "
               "(2) reduce costs (care hours, agency options), (3) use assets (sale proceeds or RM lump). "
               "Which lever do you want to try first?")
    elif code == "housing":
        keep_cost = _money_fmt(cst.get("home",0))
        rm_mo = _money_fmt(inc.get("reverse_mortgage_monthly",0))
        sale_net = _money_fmt(ast.get("home_sale_proceeds_applied",0))
        msg = (f"At current inputs: **Keep** adds home costs {keep_cost}/mo; "
               f"**Sell** shows net proceeds {sale_net} (home costs ≈ $0/mo); "
               f"**Reverse Mortgage** (monthly) adds {rm_mo}/mo. Want a side-by-side card?")
    elif code == "benefits":
        vaA = _money_fmt(inc.get("va_A",0)); vaB = _money_fmt(inc.get("va_B",0))
        msg = (f"You entered VA monthly A={vaA}, B={vaB}. Do you want an Aid & Attendance checklist "
               "to see if monthly eligibility could be higher? (informational only)")
    elif code == "explain":
        msg = (f"Monthly costs = Care {_money_fmt(cst.get('care',0))} + Home {_money_fmt(cst.get('home',0))} + "
        ...
        msg = ("We can model care hours. For example, try reducing in‑home hours by 10–20% or switching to a blended mix. "
               "Would you like me to show a what‑if table next?")
    transcript.append(("user", {"gap":"Close the funding gap", "housing":"Compare housing options", "benefits":"Maximize benefits (VA, others)", "explain":"Explain my numbers", "hours":"Tune in-home care hours"}[code]))
...
# === Tools feature flags (declared early) ===
...
# Fallback inject for Tools section
try:
    _render_tools_section_v2()

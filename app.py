...
def render_pfma():
    ...
    # Pull what we can from session for context
    s = st.session_state
    people = s.get("people", [])
    ...
    # Booking details (prefill fake data for prototyping)
    st.subheader("Booking details")
    # one-time default fill
    ...
    with col1:
        ...
        st.selectbox("Your relationship to the care recipient", ["Self","Spouse/Partner","Adult child","Relative","Friend/Neighbor","Other"], key="pfma_relationship")
        ...
        st.selectbox("Approximate age range of care recipient (optional)", ["<65","65–74","75–84","85–89","90+"], key="pfma_age_band")
    with col2:
        st.selectbox(
        ...
        st.text_area("Any must-haves or constraints? (optional)", key="pfma_notes", placeholder="e.g., near Pasadena, small community, pet-friendly")
        st.selectbox("Primary payer (optional)", ["Prefer not to say","Private pay","Long-term care insurance","Medicaid (or waiver)","VA benefit","Other / mixed"], key="pfma_payer")

    st.divider()

    # Optional follow-up (checkbox style instead of giant free-text)
    with st.expander("Optional details for your advisor"):
        st.markdown("These help your advisor prepare. Totally optional.")
        colA, colB = st.columns(2)

        with colA:
            st.multiselect("Chronic conditions (check all that apply)", [
                "Dementia / memory loss","Parkinson’s","Stroke history","Diabetes",
                "CHF / heart disease","COPD / breathing issues","Cancer (active)","Depression / anxiety","Other"
            ], key="pfma_conditions")

            st.multiselect("Activities that need support (ADLs/IADLs)", [
                "Bathing","Dressing","Toileting","Transferring / mobility","Eating / meals",
                "Medication management","Transportation","Housekeeping / laundry","Finances / bills","Companionship / safety checks"
            ], key="pfma_adls")
        ...
        with colB:
            ...
            st.selectbox("Why are you seeking care now? (optional)", [
                "Care needs increased","Caregiver burnout","Recent fall / hospitalization","Memory decline",
                "Planning ahead","Finances / affordability","Other"
            ], key="pfma_why")
    ...
    with c2:
        if st.button("Save details", key="pfma_save"):
            st.session_state.pfma_booking = {
                ...
                "why_now": s.get("pfma_why", None),
                "open_multi": s.get("pfma_open_multi", True),
            }
            st.success("Saved. Your advisor will review these before calling.")
            st.balloons()
    with c3:
        st.button("Finish", key="pfma_finish", on_click=lambda: (setattr(st.session_state, "step", "intro")))
    # end PFMA

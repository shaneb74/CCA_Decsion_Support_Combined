...
class PlannerEngine:
    ...
    def run(self, answers: Dict[str, int], conditional_answer: Optional[int] = None) -> PlannerResult:
        ...
        if assisted >= assisted_cut:
            msg = self.final_texts.get('assisted_living_recommendation', {}).get('message', 'Assisted Living recommended.')
            return PlannerResult('assisted_living', list(flags), scores, [msg])

        if in_home >= in_home_cut and assisted < assisted_cut:
            msg = self.final_texts.get('in_home_with_support', {}).get('message', 'In-Home Care recommended.')
            return PlannerResult('in_home', list(flags), scores, [msg])

        advisory = self.final_texts.get('balanced_recommendation', {}).get('message','Both options may work.')
...
class CalculatorEngine:
    ...
    def monthly_cost(self, inp: CalcInputs) -> int:
        m=self.lookups; days=int(self.settings.get('days_per_month',30))
        ...
        subtotal=int(inp.in_home_hours_per_day)*int(rate)*days+care_add+mob_add+chronic_add
        return int(round(subtotal*state_mult))

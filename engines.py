...
class PlannerEngine:
    ...
    def _evaluate(self, answers: Dict[str, int]) -> Tuple[Set[str], Dict[str, int], List[str]]:
        ...
        for idx, q in enumerate(self.qa.get("questions", []), start=1):
            ...
            for category, tlist in triggers.items():
                for t in tlist:
                    if "answer" in t and int(t["answer"]) == a:
                        flag = t.get("flag")
                        if flag:
                            flags.add(flag)
                            debug.append(f"{qk}={a} → flag {flag}")
        ...
        for rule in self.precedence:
            ...
            if self._match(rule, meta, ctx):
                ...
                if outcome is None:
                    ...
                    elif "independent" in r_lower or "no_care" in r_lower:
                        outcome = "none"
                    else:
                        outcome = "in_home"
                return rule, outcome
    ...
    def _match(self, rule_name: str, meta: Dict[str, Any], ctx: Dict[str, Any]) -> bool:
        ...
        if rule_name == "memory_care_override":
            return any_of("severe_cognitive_risk", "moderate_cognitive_decline") and any_of("no_support", "limited_support")
        ...
        return False  # Strict fallback to prevent unexpected triggers
    ...
    def _render(self, rule_name: str, flags: Set[str], scores: Dict[str, int], name: str) -> str:
        ...
        phrases = []
        issue_catalog = self.rec.get("issue_phrases", {})
        for flg in flags:
        ...
        ctx = dict(
            ...
            home_preference=("prefer to stay home" if any(n in flags for n in ["prefers_home", "strongly_prefers_home"]) else "are open to moving"),

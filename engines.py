# engines.py — Planner/Calculator engines + narrative resolver
...
class PlannerEngine:
    ...
    def __init__(self, qa_path: str, rec_path: str):
        ...
        # Order rules by precedence; fall back to file order if not provided
        ...
        # Thresholds (support both numeric and descriptive text)
        self.in_home_min = 3
        self.al_min = 6
        self.dep_min = 2
        ...
        # accept either ints or strings like ">= 6"
        def _num(v, default):
    ...
    def _decide(self, flags: Set[str], scores: Dict[str, int]) -> Tuple[str, str]:
        ...
        # Fast memory-care override (aligned with JSON intent)
    ...
    def _match(self, rule_name: str, meta: Dict[str, Any], ctx: Dict[str, Any],
        ...
        # Dependence / safety consolidation
        ...
        # Unknown rule names: allow them to pass; narrative/outcome will still render
        return True
...
# --------- Simple cost engine (thin wrapper) ---------
...
# --------- Narrative resolver (utility) ----------
def resolve_narrative(result_obj_or_dict: Any) -> str:
    """Return the best-available narrative from a PlannerResult or dict."""
    try:
        if hasattr(result_obj_or_dict, "narrative") and result_obj_or_dict.narrative:
            return result_obj_or_dict.narrative
        if isinstance(result_obj_or_dict, dict):
            for k in ("narrative", "advisory", "message", "message_rendered", "message_template"):
                v = result_obj_or_dict.get(k)
